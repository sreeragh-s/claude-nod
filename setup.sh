...
setup_claude_desktop() {
    ...
    if [ -f "$CLAUDE_CONFIG" ]; then
        ...
        if [ "$INTERACTIVE" = true ]; then
            ...
            echo ""
            print_info "Non-interactive mode: backing up and updating config..."
        fi
...
request_permissions() {
    ...
    if [ "$INTERACTIVE" = true ]; then
        ...
        echo ""
        print_info "Non-interactive mode: Please grant permissions manually when prompted"
    fi
}
...
add_to_login_items() {
    ...
    if [ "$INTERACTIVE" = true ]; then
        ...
        # Auto-add to login items in non-interactive mode
        osascript -e "tell application \"System Events\" to make login item at end with properties {path:\"$APP_PATH\", hidden:false}" 2>/dev/null || true
        print_success "Added to Login Items (auto-start enabled)"
        echo ""
        print_info "You can manage Login Items in:"
        echo "  System Settings > General > Login Items"
    fi
